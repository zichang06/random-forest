...
# 200 是可以的，
# 20 begin
trainSampleNum = int(1719692 / 2)
testSampleNum = int(429923 / 1)
...
class myForest:
    ...
    def buildTree(self, index, mylist1 = None, mylist2 = None):
        '''
        构建一棵树
        '''
        # 返回两个list
    ...
    def fitWithMultiProcess(self, sortedFeatures, label):
        '''
        多线程构造随机森林
        '''
        # self.label = label
        ...
        self.label = MG.list(label)
        self.sortedFeatures = MG.list(sortedFeatures)
        self.list_tree = MG.list(self.list_tree)
        self.list_featureIndex = MG.list(self.list_featureIndex)


        p = multiprocessing.Pool(processes=maxProcessOneTime)
        for i in range(self.n_bootstrapSamples):
             p.apply_async(self.buildTree,args=(i,))
        p.close()
        p.join()
    ...
    def predictWithMultiProcess(self, data):
        '''
        ...
        for i in range(len(self.list_tree)):
            result = p.apply_async(self.treePredict,args=(results, i))
            results[:, i] = result.get()
        p.close()
        p.join()

        # for i in range(len(self.list_tree)):
        #     process = multiprocessing.Process(target=self.treePredict,args=(results, i))
        #     process.start()
...
if __name__ == '__main__':
    ...
    clf.fitWithMultiProcess(sortedFeatures, label)
    t3 = time.time()
    ...
    #pre = clf.predictWithMultiProcess(data)
    #pre = pre.astype(int)
    t5 = time.time()
